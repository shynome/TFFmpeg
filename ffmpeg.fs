...
module ffmpeg =
    ...
    let getFFmpegPath() =
        let dirs =
            seq {
                yield (let file = System.Environment.GetCommandLineArgs()
                       Path.GetDirectoryName(file.[0]))
    ...
    type Transformer() =
        ...
        member private this.GetVideoMetadata (inputfile:string) =
            async {
                ...
                let! a = MediaFile inputfile |> f.GetMetaDataAsync |> Async.AwaitTask
                let a = if isNull a then None else Some(a)
                return a
            }
        member this.GetVideosMetadata (files: string array) =
            async {
                let! metas = files |> Array.map this.GetVideoMetadata |> Async.Parallel
